...
class CreateHrKaryawanUpdateTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('hr_karyawan_update', function (Blueprint $table) {
            $table->id();
            ...
            $table->enum('status_update', ['sudah', 'belum'])->default('belum');
            $table->timestamps();
            $table->foreign('batch_id')->references('id')->on('hr_karyawan_update_batch');
        });
    }
    ...
    {
        Schema::dropIfExists('hr_karyawan_update');
    }
}
