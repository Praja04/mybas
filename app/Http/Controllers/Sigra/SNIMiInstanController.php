...
use App\Models\Sigra\SNIMiInstan;
use App\Models\Sigra\SertifikatSNIMiInstan;
use App\Models\Attachment;

class SNIMiInstanController extends Controller
{
    public function index()
    {
        $perisahaan = Perusahaan::all();
        return view('sigra.sni-mi-instan', [
    ...
    {
        $data = [];
        $sni = SNIMiInstan::where('status', '!=', 'deleted')->get();
        ...
        foreach ($sni as $key => $d)
        {
            ...
            $sertifikat = SertifikatSNIMiInstan::where('id_varian', $d->id)->where('status', '!=', 'deleted')->orderBy('tanggal_expired', 'desc')->first();
    ...
    {
        $sertifikat = SertifikatSNIMiInstan::where('id_varian', $id)->where('status', '!=', 'deleted')->orderBy('tanggal_expired', 'desc')->get();
    ...
    {
        // dd($request->all());
        $sertifikasi = new SertifikatSNIMiInstan;
    ...
    {
        // dd($request->all());
        $sertifikasi = SertifikatSNIMiInstan::find($request->id);
        $sertifikasi->id_varian = $request->varian_id;
        $sertifikasi->nomor_sertifikat = $request->nomor_sertifikat;
        $sertifikasi->tanggal_terbit = $request->tanggal_sertifikasi;
        $sertifikasi->tanggal_expired = $request->tanggal_expired;
    ...
    {
        $sertifikasi = SertifikatSNIMiInstan::find($id);
