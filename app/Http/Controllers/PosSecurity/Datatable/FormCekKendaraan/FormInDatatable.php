        ...
        if (!empty($nopols)) {
            try {
                ...
                if ($response->successful()) {
                    ...
                    Log::info("Warehouse API vehicle/transactions/batch (FormIn) berhasil dipanggil.", [
                        'requested_nopols' => $nopols,
                        'matched_count'    => count($warehouseData),
                    ]);
                } else {
                    Log::warning("Warehouse API batch (FormIn) gagal: HTTP " . $response->status(), [
                        'body' => $response->body()
                    ]);
                }
            } catch (\Throwable $e) {
                // Fallback gracefully jika koneksi warehouse API terkendala atau offline
                Log::warning("Gagal mengambil data transaksi kendaraan dari Warehouse API: " . $e->getMessage());
            }
        }
