...
class DashboardController extends Controller
{
    ...
    {
        $workspaceId = $request->id_workspace;
        $jadwalId    = $this->getActiveJadwalId($request->id_jadwal);

        if (!$workspaceId || !$jadwalId) {
            return response()->json(['status' => 'success', 'labels' => [], 'datasets' => []]);
        }

        $departments = $this->getDepartments($workspaceId, $jadwalId);
        ...
        $jadwalTahun = (int) DB::table('5r_jadwal_penilaian')
            ->where('id_jadwal', $jadwalId)
            ->value('tahun');
    ...
    public function getDataRankPeriodeByWorkspace(Request $request)
    {
        ...
        // BOBOT SEKALI
        $bobotMap = MasterGroupJuriDepartment::whereIn('id_periode', $periodes)
            ->get()
            ->groupBy(fn ($r) => $r->id_periode.'-'.$r->id_department);
        ...
        foreach ($departments as $deptId => $deptName) {
            ...
            foreach ($periodes as $periodeId) {
                $rows = $rawNilai->get($periodeId.'-'.$deptId, collect());

                $key = $periodeId . '-' . $deptId;
        ...
        return response()->json([
            'status' => 'success',
            'labels' => $sorted->pluck('nama_department')->toArray(),
            'data'   => $sorted->pluck('nilai_akhir')->toArray()
        ]);
    }
    ...
    private function isLatestJadwal($jadwalId): bool
    {
    ...
    private function getActiveJadwalId($jadwalId)
    {
        return $jadwalId ?: DB::table('5r_jadwal_penilaian')
            ->orderByDesc('tahun')
            ->value('id_jadwal');
    }


    private function getDepartments($workspaceId, $jadwalId)
    {
        $isLatest = $this->isLatestJadwal($jadwalId);

        return DB::table('5r_master_department as d')
            ->where('d.id_workspace', $workspaceId)
            ->when($isLatest, fn ($q) => $q->where('d.is_active', 'Y'))
            ->when(!$isLatest, function ($q) use ($jadwalId) {
                $q->whereExists(function ($sub) use ($jadwalId) {
                    $sub->selectRaw(1)
                        ->from('5r_master_group as g')
   ...
   private function getNilaiRaw($jadwalId)
    ...
    private function hitungNilaiFromRaw($rows, float $bobot, int $tahun): float
    {
        ...
        $sum = $rows->sum(fn ($r) =>
            $r->total_nilai * ($r->persentase / 100)
        );
