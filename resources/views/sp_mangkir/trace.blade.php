...
@section('content')
    ...
    <div class="card shadow-sm border-0">
        <div class="card-header bg-white py-3">
            <form id="filterTraceForm" class="row g-2 align-items-center" onsubmit="event.preventDefault(); loadTraceData(1);">
                <div class="col-md-3">
                    <input type="text" id="traceSearchInput" name="search" class="form-control form-control-sm"
                        placeholder="Cari NIK, Nama, No SP, Kode...">
                </div>
                <div class="col-md-3">
                    <select id="traceStatusSelect" name="status" class="form-select form-select-sm">
                        <option value="">-- Semua Status --</option>
                        <option value="AKTIF">🟢 SP Mangkir Aktif (<= 6 Bln)</option>
                ...
                <div class="col-md-3">
                    <input type="month" id="traceBulanInput" name="bulan" class="form-control form-control-sm">
                </div>
                <div class="col-md-3">
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header bg-danger text-white py-2">
                    <h5 class="modal-title fs-6 fw-bold"><i class="ri-ban-line me-1"></i> Batalkan SP Mangkir (Cancel)
                    </h5>
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea id="cancelNotes" class="form-control" rows="3"
                            placeholder="Masukkan alasan kenapa SP Mangkir ini dibatalkan..."></textarea>
                    </div>
                </div>
            ...
            <div class="modal-content">
                <form action="{{ route('sp_pelanggaran.export') }}" method="GET" target="_blank">
                    ...
                    <div class="modal-header bg-primary text-white py-2">
                        <h5 class="modal-title fs-6 fw-bold"><i class="ri-download-2-line me-1"></i> Export Riwayat SP
                            Mangkir</h5>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"
                            aria-label="Close"></button>
                    </div>
...
@push('scripts')
    <script>
                    ...
                    $('#traceTableBody').html(`
                        <tr>
                            <td colspan="9" class="text-center py-4 text-danger">
                                <i class="ri-error-warning-line me-1"></i> Gagal memuat data SP Mangkir. Silakan coba lagi.
                            </td>
                        </tr>
                    `);
                }
        ...
        function renderTable(paginator, isAdmin, isIrRole) {
                ...
                $('#traceTableBody').html(`
                    <tr>
                        <td colspan="9" class="text-center py-5 text-muted">
                            ...
                            Belum ada data SP Mangkir yang ditemukan.
                        </td>
                    </tr>
                `);
                return;
            ...
            list.forEach((sp, index) => {
                ...
                if (sp.dates && sp.dates.length > 1) {
                    let d = sp.tanggal_pelanggaran ? new Date(sp.tanggal_pelanggaran) : null;
                    let formatted = '-';
                    if (d && !isNaN(d.getTime())) {
                        let day = String(d.getDate()).padStart(2, '0');
                        let months = ['Jan', 'Feb', 'Mar', 'Apr', 'Mei', 'Jun', 'Jul', 'Agu', 'Sep', 'Okt', 'Nov', 'Des'];
                        formatted = `${day} ${months[d.getMonth()]} ${d.getFullYear()}`;
                    }
                    dateDisplay = `<strong>${formatted}</strong><br><small class="badge bg-info text-dark" style="font-size: 10px;"><i class="ri-calendar-event-line me-1"></i>${sp.dates.length} Tanggal</small>`;
                } else if (sp.tanggal_pelanggaran) {
                ...
                let statusBadge = '';
                if (cs === 'CANCELLED') {
                    statusBadge = '<span class="badge bg-secondary sp-badge"><i class="ri-ban-line me-1"></i> CANCEL</span>';
                } else if (cs === 'REJECTED') {
                    statusBadge = '<span class="badge bg-danger sp-badge"><i class="ri-close-circle-line me-1"></i> DITOLAK</span>';
                } else if (cs === 'APPROVED') {
                    if (isExpired) {
                        statusBadge = '<span class="badge bg-dark sp-badge" title="Masa berlaku 6 bulan telah habis"><i class="ri-history-line me-1"></i> TIDAK AKTIF (EXPIRED)</span>';
                    } else if (['SP 3', 'SP III', 'SP III+'].includes(sp.jenis_pelanggaran)) {
                ...
                if (isAdmin && ['DRAFT', 'REJECTED'].includes(cs)) {
                    actions += `
                ...
                if (cs !== 'APPROVED' && !['CANCELLED', 'CANCEL_PENDING_DH', 'CANCEL_PENDING_IR', 'CANCEL_PENDING_IR_HEAD'].includes(cs)) {
                    if (isIrRole || (isAdmin && ['DRAFT', 'PENDING_DH', 'REJECTED'].includes(cs))) {
                        actions += `
                ...
                let kodeBadge = `<span class="badge bg-primary">${sp.kode_admin || ('Mangkir ' + (sp.mangkir_ke || ''))}</span>`;

                let jenisBadge = sp.jenis_pelanggaran ?
                    `<span class="badge sp-badge ${['SP 1', 'SP 2', 'SP 3'].includes(sp.jenis_pelanggaran) ? 'bg-danger' : 'bg-info text-dark'}">${sp.jenis_pelanggaran}</span>` :
        ...
        $(document).ready(function() {
            ...
            // Filter Status & Bulan Change
            $('#traceStatusSelect, #traceBulanInput').on('change', function() {
                loadTraceData(1);
            });
                    ...
                    if (res.status === 'success') {
                        ...
                        if (datesArr.length > 0) {
                            ...
                            if (datesArr.length > 1) {
                                dateDisplay =
                                    `<strong>${formattedList[0]}</strong> <span class="badge bg-info text-dark">Multi-Date (${datesArr.length} Tanggal)</span><br><small class="text-primary"><i class="ri-calendar-event-line me-1"></i>Total ${datesArr.length} Tanggal Mangkir: ${formattedList.join(', ')}</small>`;
                            } else {
