...
class ManagementController extends Controller
{
    ...
    //                         $__group = MasterGroup::where('id_department', $id_department)
    //                             ->get();
    ...
    //                                 // Menghitung total nilai dasar dari jawaban
    //                                 $nilaiDasar = $jawaban->sum('nilai'); // Total nilai dari jawaban

    //                                 // Menambahkan nilai berdasarkan is_digitalisasi ke nilaiDasar
    //                                 if ($__item->is_digitalisasi == 'Y') {
    //                                     $nilaiDasar += 28; // Tambah 28 jika is_digitalisasi = 'Y'
    //                                 } elseif ($__item->is_digitalisasi == 'N') {
    //                                     $nilaiDasar += 40; // Tambah 40 jika is_digitalisasi = 'N'
    //                                 }

    //                                 // Menghitung nilai persentase berdasarkan nilaiDasar yang sudah disesuaikan
    //                                 $nilaiPersentase = $nilaiDasar * ($__item->persentase / 100);

    //                                 // Logging untuk debugging
    //                                 \Log::info('Perhitungan Nilai:', [
    //                                     'id_group' => $__item->id_group,
    //                                     'nama_group' => $__item->nama_group,
    //                                     'persentase' => $__item->persentase,
    //                                     'is_digitalisasi' => $__item->is_digitalisasi,
    //                                     'nilaiDasar' => $nilaiDasar,
    //                                     'nilaiPersentase' => $nilaiPersentase,
    //                                 ]);

    //                                 // Total nilai adalah nilaiPersentase
    //                                 $totalNilai = $nilaiPersentase;
    //                             }
            ...
            $workspace = $workspace->map(function ($_item) {
                $_data = $_item->departments
                    ...
                    ->map(function ($item) {
                        ...
                        $periode = $periode->map(function ($_item, $key) use ($id_department) {
                            ...
                            $_group = $__group->map(function ($__item, $key) use ($id_periode) {
                                ...
                                if ($__item->jawabanGroup && $__item->jawabanGroup->status == 'approved') {
                                    ...
                                    // Terapkan bobot persentase
                                    $totalNilai = $totalNilai * ($__item->persentase / 100);
                                ...
                                $__item->totalNilai = round($totalNilai, 2); // Bulatkan
                                return $__item;
                            });
                            ...
                            // Simpan data grup dan total nilai
                            $_item->group = $_group;
                        ...
                        // Hitung rata-rata periode yang bukan nol
