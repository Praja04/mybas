...
@section('content')
                                ...
                                <div class="form-group mb-2">
                                    ...
                                    <select name="filter_jadwal" id="filter_jadwal" class="form-control">
                                        ...
                                        @foreach ($allJadwal as $_jadwal)
                                            <option @if (isset($_GET['filter_jadwal']) && $_GET['filter_jadwal'] == $_jadwal->id_jadwal) selected @endif
                                                value="{{ $_jadwal->id_jadwal }}">{{ $_jadwal->tahun }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                                ...
                                                <div class="row">
                                                    <div class="col-2">
                                                        <div class="nav nav-pills flex-column nav-pills-tab custom-verti-nav-pills text-center"
                                                            style="margin-top: 40px" role="tablist"
                                                            aria-orientation="vertical">
                                                            @foreach ($data->sortByDesc('__total') as $department)
                                                                <a class="nav-link @if ($loop->iteration == 1) active show @endif"
                                                                    style="margin-top: 8.6px"
                                                                    id="custom-v-pills-{{ $department->id_department }}-tab"
                                                                    data-bs-toggle="pill"
                                                                    href="#custom-v-pills-{{ $department->id_department }}"
                                                                    role="tab"
                                                                    aria-controls="custom-v-pills-{{ $department->id_department }}"
                                                                    aria-selected="true">
                                                        ...
                                                        <div class="tab-content text-muted">
                                                            @foreach ($data->sortByDesc('__total') as $department)
                                                                <div class="tab-pane fade @if ($loop->iteration == 1) active show @endif"
                                                                    id="custom-v-pills-{{ $department->id_department }}"
                                                                    role="tabpanel"
                                                                    aria-labelledby="custom-v-pills-{{ $department->id_department }}-tab">
                                                                    <div class="row">
                                                                                            ...
                                                                                            <tr>
                                                                                                <td>
                                                                                                    <div
                                                                                                        ...
                                                                                                        <a href="javascript:;"
                                                                                                            onClick="getDetail('{{ $periode->id_periode }}', '{{ $group->id_group }}')">
                                                                                                            <div>
                                                                                                                {{ $group->nama_group }}
                                                                                                                <i
                                                                                                        ...
                                                                                                        <div>
                                                                                                            <a target="_blank"
                                                                                                                href="{{ url('5r-system/report/download') . '/' . encrypt($department->id_department . '/' . $_GET['filter_jadwal'] . '/' . $periode->id_periode . '/' . $group->id_group) }}"
                                                                                                                data-bs-toggle="tooltip"
                                                                                                                title="Print hasil penilaian"
                                                                                                                type="button"
                                                                                                                class="btn p-0 btn-sm waves-effect waves-light shadow-none">
                                                                                                                <i
                                                                                                                    class="mdi mdi-printer mdi-24px"></i>
                                                                                                            </a>
                                                                                                        </div>
                                                                                                    </div>
                                                                                                </td>
                                                                                                ...
                                                                                                <td>{{ $group->totalNilai / ($group->persentase / 100) }}
                                                                                                </td>
                                                                                            </tr>
                                                                                        @endforeach
                                                                                    </tbody>
    ...
    <!-- Modal -->
    <div class="modal fade" id="detailModal" tabindex="-1" role="dialog" aria-labelledby="detailModalId"
        aria-hidden="true">
                            ...
                            item.forEach(function(jawaban) {
                                ...
                                if (jawaban.foto != null) {
                                    ...
                                    fotoNameArray.forEach(function(fotoName, index) {
                                        ...
                                        var temuanMatch = temuanAreas.find(t => t
                                            .foto === fotoName);
