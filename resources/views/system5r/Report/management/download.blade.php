...
<body class="pt-5 pb-5">
    <div class="text-center">
        ...
        <h3 class="text-center">PT BUMI ALAM SEGAR</h3>
                        ...
                        <table class="table table-striped">
                            ...
                            <tbody>
                                <tr class="pas-background-color">
                                    <th class="text-white px-1"
                                        style="border-width: 1px solid #fff; width: 5% !important">GROUP</th>
                                ...
                                @foreach (['RINGKAS', 'RAPI', 'RESIK', 'RAWAT', 'RAJIN'] as $jenis)
                                    ...
                                    @foreach ($__pertanyaan as $_pertanyaan)
                                        <tr>
                                            @if ($loop->first)
                                                <td class="p-0"
                                                    style="vertical-align: middle; font-size: 10px; font-weight: bold; text-align: center; background-color: {{ $colors[$loop->parent->iteration - 1] }}; color: {{ $textColors[$loop->parent->iteration - 1] }}"
                                                    rowspan="{{ count($__pertanyaan) }}">
                                            ...
                                            <td>
                                                <div>
                                                    <h6>NILAI <span title="Wajib diisi" class="text-danger">*</span>
                                                    </h6>
                                                    @if ($jawaban != null)
                                                        @php
                                                            $__jawaban = $jawaban
                                                                ->where('id_pertanyaan', $_pertanyaan->id_pertanyaan)
                                                                ->first();
                                                            $nilai = $__jawaban != null ? $__jawaban->nilai : '';
                                                        @endphp
                                                        <select disabled class="form-control" style="width: 100px"
                                                            name="nilai[{{ $_pertanyaan->id_pertanyaan }}]">
                                                            <option value="">PILIH</option>
                                                            <option @if ($nilai == '1') selected @endif
                                                                value="1">1</option>
                                                            <option @if ($nilai == '2') selected @endif
                                                                value="2">2</option>
                                                            <option @if ($nilai == '3') selected @endif
                                                                value="3">3</option>
                                                            <option @if ($nilai == '4') selected @endif
                                                                value="4">4</option>
                                                        </select>
                                                    @else
                                                        <select required
                                                ...
                                                <div class="mt-3">
                                                    ...
                                                    @if ($jawaban != null)
                                                        ...
                                                        @if ($foto != null)
                                                            @php
                                                                ...
                                                                $temuanList = $__jawaban->temuan ?? collect();
                                                            @endphp

                                                            @foreach ($fotoArray as $_foto)
                                                                @php
                                                                    $_foto = trim($_foto);
                                                                    $temuan = $temuanList->firstWhere('foto', $_foto);
                                                                @endphp
                                                    ...
                                                    @else
                                                        ...
                                                        <div class="textarea_image_container" style="display: none"
                                                            id="{{ $_pertanyaan->id_group }}-{{ $_pertanyaan->id_pertanyaan }}_image_container">
                                                    ...
                                                    @if ($jawaban != null)
                                                        @php
                                                            $__jawaban = $jawaban
                                                                ...
                                                                ->first();
                                                            $keterangan =
                                                                $__jawaban != null ? $__jawaban->keterangan : '-';
                                                        @endphp
                                                        <p>{{ $keterangan }}</p>
                                                    @endif
                                                </div>
                    ...
                    @if ($jawabanGroup == null)
                        <div class="mt-3">
                            <button type="submit" form="form-{{ $group->id_group }}"
                                class="btn btn-full btn-success waves-effect">
