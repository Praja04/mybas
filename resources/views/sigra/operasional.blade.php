                ...
                <div class="card card-custom">
                    <div class="card-header flex-wrap border-0 pt-6 pb-0">
                        <div class="card-title">
                            <h3 class="card-label">SIGRA SERTIFIKASI OPERASIONAL
                                <span class="d-block text-muted pt-2 font-size-sm">Manage data sertifikasi operasional</span>
                            </h3>
                        </div>
                    ...
                    <div class="card-body">
                        <table id="operasional" class="table table-hover">
                            <thead>
                                <tr>
                                    <th width="2%">#</th>
                                    <th>PT</th>
                            ...
                            <tbody>
                                {{-- @foreach ($sertifikasi_operasional as $key => $sertifikasi)
                                <tr>
                                    <td>{{ $key+1 }}</td>
                                    <td>{{ $sertifikasi['nama_perusahaan'] }}</td>
                                    <td>
                                        <a class="text-hover-dark" href="javascript:" onClick="showSertifikasi('{{ $sertifikasi['id_operasional'] }}','{{ $sertifikasi['nama_perizinan'] }} - {{ $sertifikasi['nomor_perizinan'] }}')">
                                            <i class="fa fa-archive text-danger font-size-sm"></i>
                                            {{ $sertifikasi['nama_perizinan'] }}
                                        </a>
                                    </td>
                                    <td>{{ $sertifikasi['nomor_perizinan'] }}</td>
                                    <td>{{ $sertifikasi['sertifikasi'] != null ? formatTanggalIndonesia($sertifikasi['sertifikasi']->tanggal_sertifikasi) : '-' }}</td>
                                    <td>{{ $sertifikasi['sertifikasi'] != null ? formatTanggalIndonesia($sertifikasi['sertifikasi']->tanggal_expired) : '-' }}</td>
                                    <td>{{ $sertifikasi['sertifikasi'] != null ? $sertifikasi['sertifikasi']->status : '-' }}</td>
                                    <td>{{ $sertifikasi['sertifikasi'] != null ? $sertifikasi['sertifikasi']->remarks : '-' }}</td>
                                    <td>{{ $sertifikasi['sertifikasi'] != null ? $sertifikasi['sertifikasi']->tahun : '-' }}</td>
                                </tr>
                            @endforeach --}}
                            </tbody>
                        </table>
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">
                        <span id="operasional-title"></span> <input id="perizinan-status" data-size="small"
                            ...
                            <div class="card-body">
                                <form action="{{ url('/') }}" id="form-create-sertifikat">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <label class="col-2 col-form-label text-right" for="dokumen-asli">Dokumen
                                            Asli</label>
                                        <div class="col-4">
                                    ...
                                    <div class="form-group row">
                                        <label class="col-2 col-form-label text-right" for="remarks">Remarks</label>
                                        <div class="col-4">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="edit-perizinan-form">
                        ...
                        <div class="form-group row">
                            <label class="col-3 col-form-label text-right" for="edit-nomor-perizinan">Nomor
                                Perizinan</label>
                            <div class="col-9">
                                <input required name="nomor_perizinan" placeholder="Nomor Perizinan" class="form-control"
                                    type="text" value="" id="edit-nomor-perizinan">
                            </div>
                        </div>
                        <div class="form-group row">
                            <div class="col-3"></div>
...
@push('scripts')
    ...
    <script type="text/javascript">
        ...
        function editSertifikasi(id) {
            ...
            $.ajax({
                url: "{{ url('/sigra/operasional/sertifikasi/get') }}/" + id,
                type: "GET",
                dataType: "JSON",
                success: function(response) {
                    ...
                    console.log(data);
                    $('#id').val(data.id);
                    $('#tanggal-sertifikasi').val(data.tanggal_sertifikasi);
                    $('#tanggal-expired').val(data.tanggal_expired);
                    $('#perizinan').val(data.perizinan);
                    $('#dokumen-asli').val(data.dokumen_asli);
        ...
        function deleteSertifikasi(id) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: "{{ url('/sigra/operasional/sertifikasi/delete') }}/" + id,
                        type: "DELETE",
                        dataType: "JSON",
                        success: function(response) {
                            Swal.fire(
                            ...
                            getSertifikasi($("#operasional-id").val());
                            operasional_table.ajax.reload();
                        }
                    })
                }
            });
        ...
        myDropzone5.on("sending", function(file, xhr, formData) {
            ...
            formData.append('transaction_type', 'perizinan_oprasional');
        });
        ...
        $('#create-perizinan-form').on('submit', function(e) {
            ...
            $.ajax({
                url: '{{ url('sigra/operasional/store') }}',
                type: 'POST',
                dataType: 'JSON',
                data: $(this).serialize(),
                success: function(response) {
        ...
        $('#edit-perizinan-form').on('submit', function(e) {
            ...
            $.ajax({
                url: "{{ url('sigra/operasional/update') }}",
                type: 'POST',
                dataType: 'JSON',
                data: $(this).serialize(),
                success: function(response) {
        ...
        function showDocuments(id) {
            ...
            $.ajax({
                url: "{{ url('sigra/operasional/get-attachments') }}/" + id,
                type: "GET",
                dataType: "JSON",
                success: function(response) {
        ...
        function showSertifikasi(id, title, status) {
            $('#operasional-title').text(title);
            $("#operasional-id").val(id);
        ...
        function getSertifikasi(id) {
            ...
            $.ajax({
                url: '{{ url('sigra/operasional/get-sertifikasi') }}/' + id,
                dataType: 'JSON',
                type: 'GET',
                success: function(response) {
                    if (response.data.length == 0) {
                    ...
                    $.each(response.data, function(key, val) {
                        ...
                        var row = '' +
                            '<tr>' +
                            '<td>' + (key + 1) + '</td>' +
                            '<td>' + formatTanggalIndonesia(val.tanggal_sertifikasi) + '</td>' +
                            '<td>' + formatTanggalIndonesia(val.tanggal_expired) + '</td>' +
                            '<td>' + val.perizinan + '</td>' +
                            '<td>' + val.dokumen_asli + '</td>' +
        ...
        function getPerizinanOperasional() {
            operasional_table = $('#operasional').DataTable({
                "ajax": "{{ route('sigra.operasional.get-all') }}",
                "paging": false,
                "responsive": true,
                "dom": '<"toolbar">frtip',
                "order": [
            ...
            $('#operasional_filter').append(
                "<button onClick='exportExcel()' class='btn btn-outline-success float-left btn-sm'><i class='la la-file-excel-o'></i>Export excel <i class='la la-download'></i></button>"
            )
        }
        ...
        function deleteItem(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    // Hapus data dari server
                    $.ajax({
                        url: "{{ url('/sigra/operasional/delete') }}/" + id,
                        type: "DELETE",
                        dataType: "JSON",
                        success: function(response) {
                            Swal.fire(
        ...
        function exportExcel() {
            ...
            //     'success'
            // );
            window.open('{{ url('/sigra/operasional/export') }}', '_blank');
        }
        ...
        $('#perizinan-status').on('switchChange.bootstrapSwitch', function(e, data) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: "{{ url('/sigra/operasional/set-status') }}",
                        type: "POST",
                        dataType: "JSON",
                        data: {
                            id: id,
