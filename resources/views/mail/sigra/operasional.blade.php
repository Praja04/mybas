...
<head>
    ...
    <title>(FYI) Sigra Notification Operasional</title>
</head>
<body>
    <h3 style="text-align: center">Sigra Notification Sertifikasi Operasional <span style="font-size: 5px; color: #eee">{{ date('YmdHis') }}</span></h3>
            ...
            <tbody>
                @foreach ($sertifikasi as $key => $sertifikat)
                    <tr style="background-color: {{ $sertifikat->due_date > 0 ? '#FFC857' : '#C5283D' }}">
                        ...
                        <td style="border: 1px solid #333;padding: 8px;">{{ $sertifikat->nomor_perizinan }}</td>
                        <td style="border: 1px solid #333;padding: 8px;">{{ @formatTanggalIndonesia($sertifikat->tanggal_expired) }}</td>
